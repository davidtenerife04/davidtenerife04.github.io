...
void performCalculation() {
    if (pending_op != 0) {
        ...
        numA = result; // Almacena para operaciones secuenciales
        history = L""; // Borra historial tras el '='
        pending_op = 0;
        next_entry_clears = true;
    }
}

// El motor de redimensionamiento dinámico CORREGIDO
void LayoutControls(HWND hwnd) {
    ...
    // Márgenes y padding fijos
    int margin = 5;
    int hHistory_h = 25; // Alto fijo para el historial
    int hDisplay_h = 50; // Alto fijo para la pantalla principal
    
    // Posicionar pantallas (Resultado e Historial) - Adaptando el ancho
    SetWindowPos(hStaticHistory, NULL, margin, margin, width - (2 * margin), hHistory_h, SWP_NOZORDER);
    SetWindowPos(hDisplay, NULL, margin, margin + hHistory_h + 2, width - (2 * margin), hDisplay_h, SWP_NOZORDER); // Añadido +2 para espacio

    // Calcular tamaño y origen de la cuadrícula de botones
    int grid_start_y = margin + hHistory_h + hDisplay_h + margin + 10; // Espacio extra
    ...
    // Reposicionar botones en el grid de forma ESTRICTA
    for (int row = 0; row < NUM_ROWS; ++row) {
        for (int col = 0; col < NUM_COLS; ++col) {
            int index = row * NUM_COLS + col; // Índice lineal estricto
            HWND hBtn = hGridButtons[index];
            if (hBtn) {
                SetWindowPos(hBtn, NULL, margin + (col * btn_w), grid_start_y + (row * btn_h), btn_w - 1, btn_h - 1, SWP_NOZORDER); // -1 para borde sutil
            }
        }
    }
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Pantallas (Creadas a 0,0 - LayoutControls las posiciona)
        ...
        // Crear botones según el orden estricto del grid y guardar HWND
        for (int i = 0; i < NUM_ROWS * NUM_COLS; ++i) {
            HWND hBtn = CreateWindow(L"BUTTON", buttons[i].label, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 0, 0, 0, 0, hwnd, (HMENU)buttons[i].id, NULL, NULL);
            SendMessage(hBtn, WM_SETFONT, (WPARAM)hFontMain, TRUE);
            hGridButtons[i] = hBtn; // Guardar el HWND en el array global
        }
    ...
    case WM_SIZE: {
        // Llamar a la función que corrige el layout
        LayoutControls(hwnd);
    ...
    case WM_COMMAND: {
        ...
        if ((id >= ID_BTN_0 && id <= ID_BTN_9) || id == ID_BTN_DOT) {
            ...
            wchar_t charToAdd = 0;
            if (id == ID_BTN_DOT) {
                if (current_entry.find(L'.') == std::wstring::npos) // Solo un punto
                    charToAdd = L'.';
            } else {
            ...
            if(current_entry.empty() || current_entry == L".") current_entry = L"0" + current_entry; // Arreglar ".5" a "0.5"
            SetWindowText(hDisplay, current_entry.c_str());
        }
        ...
        // --- Manejo de Operaciones (+, -, *, /) ---
            ...
            if(pending_op != 0 && !next_entry_clears) {
                performCalculation();
                SetWindowText(hDisplay, current_entry.c_str());
            }
        ...
        // --- Controles Especiales (C, CE, Backspace, Signo) ---
        else if (id == ID_BTN_C) {
            ClearAll();
        }
    ...
    case WM_GETMINMAXINFO: {
        // Establecer tamaño mínimo para no romper el layout
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, CLASS_NAME, L"Calculadora Responsiva V3.0", WS_OVERLAPPEDWINDOW,
